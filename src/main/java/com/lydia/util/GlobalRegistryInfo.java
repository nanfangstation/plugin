...
/**
 * @author Lydia
 * @since 2020-03-01 下午3:13
 **/
public class GlobalRegistryInfo {
     ...
     * 全局插件安装次数
     */
    private static Map<String, PluginOperationInfo> operatorPluginInfos = new HashMap<>();
    ...
    public static synchronized void addOperatorPluginInfo(String pluginId,
                                                          PluginOperationInfo.OperationType operatorType,
                                                          boolean isLock) {
        PluginOperationInfo operatorPluginInfo = operatorPluginInfos.get(pluginId);
        if (operatorPluginInfo == null) {
            operatorPluginInfo = new PluginOperationInfo();
            operatorPluginInfos.put(pluginId, operatorPluginInfo);
        }
        operatorPluginInfo.setOperationType(operatorType);
     ...
     * @return 操作插件类型
     */
    public static synchronized PluginOperationInfo getPluginInstallNum(String pluginId) {
